        ...
        (JNIEnv *env, jclass jc, jstring path)
{
    ...
    PyRun_SimpleString("import print");
...
//test
JNIEXPORT jstring JNICALL Java_com_jventura_pybridge_PyBridge_printStr
        (JNIEnv *env, jclass jc){

        PyObject* myModuleString = PyUnicode_FromString((char*)"print");
        PyObject* myModule = PyImport_Import(myModuleString);

        //func
        PyObject* myFunction = PyObject_GetAttrString(myModule, (char*)"print_str");

        //args
        PyObject* pyParams = PyTuple_New(1);
        PyObject* arg1 = Py_BuildValue("s","file");
        //PyObject* arg2 = Py_BuildValue("s","path");
        PyTuple_SetItem(pyParams,0, arg1);
        //PyTuple_SetItem(pyParams,1, arg2);

        PyObject* myResult = PyObject_CallObject(myFunction, pyParams);
        char *myResultChar = PyUnicode_AsUTF8(myResult);

        return (*env)->NewStringUTF(env, myResultChar);
}
